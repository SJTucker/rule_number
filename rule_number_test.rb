require 'minitest/autorun'
require_relative 'rule_number'
require 'pry'

describe RuleNumber do

  it "integers" do
    sorted = %w{2 3 4}
    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "numerics" do
    sorted = %w{
      6.1 
      6.2 
      6.10
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "alphas" do
    sorted = %w{
      6.1(a) 
      6.1(b)
      6.1(c)
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "different levels" do
    skip
    sorted = %w{
      6A
      6.1 
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)

    sorted = %w{
      6.1
      6.1(a)
      6.1(a)(i)
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "romans" do
    skip
    sorted = %w{
      6.1(i) 
      6.1(ii) 
      6.1(iii) 
      6.1(iv) 
      6.1(v) 
      6.1(vi) 
      6.1(vii) 
      6.1(viii) 
      6.1(ix) 
      6.1(x) 
      6.1(xiv) 
      6.1(xv) 
      6.1(xvi) 
      6.1(xix) 
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "repeated" do
    skip
    sorted = %w{
      6.1(a) 
      6.1(b) 
      6.1(aa) 
      6.1(bb) 
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "lots of text" do
    skip
    sorted = [
      "NFA Compliance Rule 2-41",
      "NFA Interpretive Notice #9062",
      "NFA Registration Rule 101",
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "dash" do
    skip
    sorted = [
      "NFA Compliance Rule 2-41",
      "NFA Compliance Rule 2-42",
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "pound sign" do
    skip
    sorted = [
      "NFA Compliance Rule #2",
      "NFA Compliance Rule #3",
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "extra spaces" do
    skip
    sorted = [
      'NFA Compliance Rule 2-33',
      'NFA Compliance  Rule 2-34',
      'NFA Compliance Rule 2-34(a)'
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "numbers at the end" do
    skip
    sorted = [
      'Financial Requirements Section 4',
      'Financial Requirements Section 7',
      'Financial Requirements Section 13',
      'Financial Requirements Section 13(b)',
      'Financial Requirements Section 15',
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "all dots" do
    skip
    sorted = %w{
      506
      506.A
      506.B
      515
      515.B
      515.B.1
      515.B.2
      515.B.3
      515.C
      515.E
      523
    }

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "more dots" do
    skip

    sorted = %w{
      49.20(b)(3)
      49.20(b)(4)
      49.20(c)(1)(i)(A)
      49.20(c)(1)(i)(B)
      49.20(c)(1)(i)(C)
      49.20(c)(1)(ii)
      49.20(c)(2)
      49.20(c)(3)
      49.20(c)(4)
      49.20(c)(5)
      49.20(d)
      49.21
      49.21(a)(1)
      49.21(b)(1)
      49.21(b)(2)
      49.21(c)
      49.22(b)(1)
    }

    shuffled = %w{
      49.20(c)(3)
      49.21
      49.20(c)(1)(i)(B)
      49.21(b)(2)
      49.20(c)(2)
      49.20(c)(1)(i)(A)
      49.20(b)(3)
      49.21(a)(1)
      49.20(b)(4)
      49.21(b)(1)
      49.20(c)(4)
      49.20(c)(1)(i)(C)
      49.22(b)(1)
      49.20(d)
      49.20(c)(5)
      49.21(c)
      49.20(c)(1)(ii)
    }

    assert_equal sorted, shuffled.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "cfr" do
    skip
    sorted = [
      '17 CFR 1.23(b)',
      '17 CFR 4.23(a)(12)',
      '17 CFR 5.23(b)',
      '17 CFR 31.23(b)',
      '17 CFR 49.23(b)',
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end

  it "punctuation" do
    skip
    sorted = [
      'One, Too',
      'One, Tree',
    ]

    assert_equal sorted, sorted.reverse.collect{|number| RuleNumber.new(number)}.sort.map(&:to_s)
  end
end

